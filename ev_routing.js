const application = {
  ...
  function addRouteSummary(route) {
    ...
    if (fallbackMode)
      appendHeading(summary, 'Showing standard route (EV charging plan unavailable).', 'h4');
  ...
  function calculateRoute(finishResults) {
    ...
    calculateLongDistanceEVRoute({
        ...
        minChargeAtChargingStopsInkWh: minChargeAtChargingStopsInkWh,
      ...
      .catch(function(error) {
        ...
        if (shouldUseFallbackRoute(error)) {
          fallbackMode = true;
          displayMessage('EV route unavailable for this distance. Trying standard route...');
  ...
  function calculateFallbackRoute() {
    ...
    .then(function(result) {
      const normalized = normalizeFallbackRouteData(result);
  ...
  function displayRoute(results) {
    if (results == null || !Array.isArray(results.routes) || results.routes.length === 0) {
  ...
  function normalizeFallbackRouteData(results) {
    if (results == null || !Array.isArray(results.routes))
      return null;
    ...
    results.routes.forEach(function(route) {
      ...
      route.legs.forEach(function(leg) {
        ...
        leg.points = leg.points
          .map(convertPoint)
          .filter(function(point) { return point != null; });
      });
    });

    results.toGeoJson = function() {
      const coordinates = [];
  ...
  function convertPoint(point) {
    if (point == null)
      return null;
    if (point instanceof tt.LngLat)
      return point;
    if (point.hasOwnProperty('lng') && point.hasOwnProperty('lat'))
      return new tt.LngLat(point.lng, point.lat);
    if (point.hasOwnProperty('longitude') && point.hasOwnProperty('latitude'))
      return new tt.LngLat(point.longitude, point.latitude);
    return null;
  }
  ...
  function shouldUseFallbackRoute(error) {
    if (error == null)
      return false;

    const text = String(error).toUpperCase();
    return text.includes('NO_ROUTE_FOUND') || text.includes('NO VALID ROUTE');
  }